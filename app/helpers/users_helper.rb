module UsersHelper
  ...
  def users_as_json
    ...
    @users.map do |user|
    {
      ...
      cancel_link: ((superadmin? || admin?) && !@memberships[user.id].active?)? member_path(@memberships[user.id]) : "",
      manage_link: ((superadmin? || admin?) && user != current_user && @memberships[user.id].active?)? toggle_manager_member_path(@memberships[user.id]) : "",
